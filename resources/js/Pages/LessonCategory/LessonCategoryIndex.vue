            ...
            <div class="col-md-12">
                <div class="card card-custom rdp_statistic_mg">
                    ...
                    <div class="card-body">
                        ...
                        <Link :href="route('metronic.lesson-category.create')" as="button" method="get" class="btn btn-success mb-5">Создать категорию</Link>

                        <v-server-table
                            :url="url"
                            :columns="columns"
                            :options="options"
                            ref="lessons-category-table"
                        >
                            <template v-slot:actions="{row}">
                                <Link :href="route('metronic.lesson-category.edit', {id: row.id})" method="get">
                                ...
                                <a href="javascript:;" @click="deleteLessonCategory(row)">
...
export default {
    ...
    data() {
        return {
            url: '/dashboard/category/paginate',
            columns: [
                'id',
                'name',
                'description',
    ...
    methods: {
        deleteLessonCategory(row) {
            Swal.fire({
                title: 'Удалить категорию?',
                text: "Выбранная категория будет удалена",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    axios.delete('/dashboard/category/' + row.id).then(response => {
                        this.$notify({
                            group: 'foo',
                            type: 'success',
                        ...
                        this.$refs['lessons-category-table'].refresh();
                    });
                }
            })
        },
